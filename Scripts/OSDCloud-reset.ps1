...
<#
.SYNOPSIS
    Creates a recovery partition, installs OSDCloud WinPE, and boots to it for OS reinstallation.

.DESCRIPTION
    This script creates a recovery partition on the primary disk, downloads and configures OSDCloud WinPE,
    sets up the boot configuration, and restarts the machine to boot into WinPE for OS reinstallation.
    The script is designed to be run remotely via irm and iex.
...
# Function to create a recovery partition
function New-RecoveryPartition {
    ...
    Write-Host "Creating recovery partition on Disk $DiskNumber..."
    ...
    $freeSpaceMB = ($disk.LargestFreeExtent / 1MB)
    if ($freeSpaceMB -lt $PartitionSizeMB) {
        Write-Error "Insufficient free space on Disk $DiskNumber. Required: $PartitionSizeMB MB, Available: $freeSpaceMB MB."
        exit 1
    }
...
try {
    ...
    # Step 1: Create recovery partition
